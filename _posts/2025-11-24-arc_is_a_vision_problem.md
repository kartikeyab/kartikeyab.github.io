...
If you don’t know ARC yet, it’s one of the most interesting benchmarks in AI. Each task is a tiny colored grid puzzle. You’re given a few (input → output) examples and asked to infer the transformation rule and apply it to a held-out input. The rules are deliberately “human-ish”: mirror this pattern, move the shape up, fill the enclosed region, copy the blue object everywhere there’s green, etc. 
